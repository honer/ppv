               ...
               for (int i = 0 ; i < Command.MAX_CONNECT_COUNT_TO_PROXY; i++)
               {
                   int count = 0 ;
                    ...
                    while(true)
                    {
                        ...
                        if(count == 100) // 已經發送10秒但尚未得到回應,所以強制不等待接收,可視為此代理伺服器已離線
                        {
                        ...
                        if (Command.Socket_To_Proxy_Input_Stream[i].available() == Command.stack_request_data_to_proxy_of_check[i].get_length() )
                        {                          
